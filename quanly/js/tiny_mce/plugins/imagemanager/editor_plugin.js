				...
				ed.addCommand('mceInsertImage', function(u, v) {
					...
					mcImageManager.browse(tinymce.extend(s, {
						oninsert : function(o) {
							...
							var urlRoot = o.focusedFile.url;
							if ((urlRoot.indexOf(".docx") != -1) ||
							(urlRoot.indexOf(".doc") != -1) || 
							(urlRoot.indexOf(".ppt") != -1) ||
							(urlRoot.indexOf(".pptx") != -1) || 
							(urlRoot.indexOf(".xls") != -1) || 
							(urlRoot.indexOf(".xlsx") != -1) ||
							(urlRoot.indexOf(".rar") != -1) ||
							(urlRoot.indexOf(".zip") != -1))
							{
								files = urlRoot.split(".");								
								...
								ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(
									ed.getParam('imagemanager_insert_template', '<a href="{$url}" target="_blank"><img src="'+BASE_URL+'js/tiny_mce/images/'+filetype+'.png" />'),
									o.focusedFile,
									{
							...
							else if (urlRoot.indexOf(".swf") != -1)
							{
								ed.execCommand('mceInsertContent', false, mcImageManagerPlugin.replace(
									ed.getParam('imagemanager_insert_template', '{$url}'),
									o.focusedFile,
									{
										urlencode : function(v) {
											return escape(v);
